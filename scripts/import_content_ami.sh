...
# Change this to reflect OpenShift version of the content AMI
OCP_VER="4.13.13"
...
CONTENT_IMPORT_ID=$( aws ec2 import-snapshot --disk-container "file:///tmp/containers.json" |  jq -r '.ImportTaskId')
...
do
  ...
  x=$(aws ec2 describe-import-snapshot-tasks --import-task-ids ${CONTENT_IMPORT_ID} | jq -r '.ImportSnapshotTasks[0].SnapshotTaskDetail.Status')
  sleep 5
done

CONTENT_SNAPSHOT=$(aws ec2 describe-import-snapshot-tasks --import-task-ids ${CONTENT_IMPORT_ID} | jq -r '.ImportSnapshotTasks[0].SnapshotTaskDetail.SnapshotId')
...
# Register the AMI
CONTENT_AMI=$(aws ec2 register-image \
  --output text \
